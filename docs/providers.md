...
**Per-model reasoning is never projected from the session `thought_level` set.** The session `thought_level` option describes the currently selected model only, so Paseo attaches it to the current model and leaves other models without projected choices. An ACP agent can declare per-model truth in `ModelInfo._meta`: `reasoning: false` for uncontrollable models, `thoughtLevels: string[]` (alias `reasoningLevels`) for the model's valid effort ids, and `defaultThoughtLevel` (alias `defaultReasoningLevel`) for its authoritative default. Without `_meta`, use the client's `modelTransformer` refinement hook to supply per-model contracts from provider-native sources. Live sessions additionally report their current `thought_level` set via `runtimeInfo.extra.liveThinkingOptions` so live controls always reflect the running model.
